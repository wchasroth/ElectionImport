...
function makeVoteForCache(AlfredPDO $pdo): array {
   $result = $pdo->run("SELECT org, office FROM title26 WHERE seats=1");
   $voteForCache = [];
...
function getDistrictFromDb(AlfredPDO $pdo, string $org, string $region, string $countyNum): int {
   ...
   if (Str::startsWith($org, "vil")) {
      ...
      $sql = "SELECT DISTINCT id FROM village WHERE simplename='$name' AND county_id=$countyNum";
   }
   else if (Str::startsWith($org, "city")) {
      $name = NameSimplifier::simplifyJurisdictionName($region);
      $sql = "SELECT DISTINCT id FROM jurisdiction WHERE simplename='$name' AND type='c' AND county_id=$countyNum";
   }
   else if (Str::startsWith($org, "town")) {
      $name = NameSimplifier::simplifyJurisdictionName($region);
      $sql = "SELECT DISTINCT id FROM jurisdiction WHERE simplename IN ('$name', '$name charter') AND type='t' AND county_id=$countyNum";
   }
   else if (Str::startsWith($org, "schl")) {
      $name = NameSimplifier::simplifySchoolName($region);
      $sql = "SELECT DISTINCT id FROM school_district WHERE simplename='$name' AND county_id=$countyNum";
   }
   else if (Str::startsWith($org, "comcol")) {
      $name = NameSimplifier::simplifyCommCollegeName($region);
      $sql = "SELECT DISTINCT id FROM comm_college26 WHERE simplename='$name'";
   }
   else {
